...
public class hash_SplashTable {
	...
	public hash_SplashTable(int numElementsLog, int numElemBucket,
	...
	private void createHashFunctions() {
		for (int i = 0; i < this.numHashFunctions; i++) {
			hashFunctions[i] = new hash_SplashTable.HashFunction(getRandom(),
					numBuckets);
		}
	}
	 ...
	 */
    private void build(int tries, hash_KeyValue data) {
     ...
     * @param i
     */
    private void reInsert(int tries, hash_KeyValue data,int i){    	
    	final int hash = (int)hashFunctions[i].hash(data.getKey());
    	...
    	if (tries > numReinsertions) {    		
    		...
    		hash_KeyValue elementToReinsert = this.dataArray.tryReInsert( hash, data);
	...
	public static void main(String[] args){
		...
		hash_SplashTable splashTable = new hash_SplashTable(numElementsLog,
		...
		try{
			while (inFile.hasNext()) {
				...
				splashTable.build(0, new hash_KeyValue(key, value));
			}
		}catch(NumberFormatException e1){
	...
	private void dump(String dumpFileName){
        ...
        List<Queue<hash_KeyValue>> list = dataArray.hashTable;
        for (Iterator<Queue<hash_KeyValue>> iterator = list.iterator(); iterator.hasNext();) {
			Queue<hash_KeyValue> queue = (Queue<hash_KeyValue>) iterator.next();
			int i = 0;
			for (Iterator<hash_KeyValue> iterator2 = queue.iterator(); iterator2.hasNext()||i < numElemBucket;) {
				hash_KeyValue keyValue = null;
				if(iterator2.hasNext())
					keyValue = (hash_KeyValue) iterator2.next();
				if(keyValue != null){
	...
	private static final class DataArray {
		private List<Queue<hash_KeyValue>> hashTable;
		private int numElemBucket;
		public DataArray(int numBuckets, int numElemBucket) {
			hashTable = new ArrayList<Queue<hash_KeyValue>>(numBuckets);
			for (int i = 0; i < numBuckets; i++) {
				hashTable.add(i, new LinkedList<hash_KeyValue>());
			}
			this.numElemBucket = numElemBucket;
		}
		 ...
		 */
		public boolean insertElement(int index, hash_KeyValue data) {
		...
		public hash_KeyValue tryReInsert(int index, hash_KeyValue data) {
			// Its known that the corresponding bucket is full so move the data
			// around
			hash_KeyValue elementInsertedFirst = hashTable.get(index).poll();
		...
		public int findKey(int index, int key) {
			...
			Iterator<hash_KeyValue> iterator2 = hashTable.get(index).iterator();
			hash_KeyValue keyValue = (iterator2.hasNext())? iterator2.next(): null;
